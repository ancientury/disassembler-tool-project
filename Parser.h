...
{
public:
    int Disassemble(unsigned char *bytes, unsigned int max, int offset, QString &output);
    int Decode(const INSTRUCTION *instructions, unsigned char *bytes, unsigned int max, int offset, QString &output);

private:
    unsigned char *base;
    static inline unsigned char opcode;
...
private:
    static inline char modRM_mod;
    static inline char modRM_reg;
    static inline char modRM_rm;

private:
    static inline const char* const register_mnemonics8[] = {"al", "cl", "dl", "bl", "ah", "ch", "dh", "bh"};
    static inline const char* const register_mnemonics16[] = { "ax", "cx", "dx", "bx", "ax", "cx", "dx", "bx" };
    static inline const char* const register_mnemonics32[] = { "eax", "ecx", "edx", "ebx", "esp", "ebp", "esi", "edi" };
    static inline const char* const sib_base_mnemonics[] = { "[eax", "[ecx", "[edx", "[ebx", "[esp", "[ebp", "[esi", "[edi" };
    static inline const char* const sib_scale_mnemonics[] = { "*1", "*2", "*4", "*8" };

    static constexpr const INSTRUCTION standard_instructions[] = {
        { 1, BYTE, "add ", 2, {RM, R} }, // 0
        ...
        { 0, 0, "std", 0 }, // FD
        { 1, BYTE, "push ", 1, {RM} }, // FE
        { 1, DWORD, "push ", 1, {RM} } // FF
    };

    static constexpr const INSTRUCTION extended_instructions[] = {
        { 1, WORD, "sldt ", 1, {RM} }, // 0
        { 1, 0, "sgdtd ", 1, {RM} }, // 1
