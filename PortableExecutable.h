#ifndef PORTABLEEXECUTABLE_H
#define PORTABLEEXECUTABLE_H
...
public:
    ~PortableExecutable();
    PortableExecutable(const QString& filename);
    ...
    int GetEntrySectionIndex();
    unsigned long GetEntryPointOffset();
    unsigned long GetEntryPointVA();

    Section* GetSection(int sectionIndex);
    unsigned char* GetEPFirstBytes();

    QList<QStringList> FindCodeCaves(int minimumSize);

};

#endif // PORTABLEEXECUTABLE_H
